...
game.import('mode',function(lib,game,ui,get,ai,_status){
	return {
		...
		translate:{
			...
			wuyashenxiang:'厄音',
			wuyashenxiang_info:'距离3格以内的角色在其回合结束后，若体力值不大于1，令其回复一点体力，然后将牌堆中的一张延时策略牌置于其判定区',
			...
			lingdong:'灵动',
			lingdong_info:'结束阶段，你可以移动X个格，X为你回合内出冲的次数',
			lianshe:'箭舞',
			lianshe_info:'当你于一个回合中首次使用冲时，你可以摸一张牌；在你的回合内，每当你使用一张不是冲的牌，你可以额外使用一张冲',
			zhiming:'穿杨',
			zhiming_info:'锁定技，当你使用冲造成伤害时，若你不在目标的攻击范围内，此伤害+1',
			sanjiansheji:'散箭',
			sanjiansheji_info:'你可以将两张冲当冲使用，此冲可以指定距离你5格以内任意名目标',
			guanchuan:'强弩',
			guanchuan_info:'当你使用冲指定惟一的目标后，可将攻击射线内的其他角色也加入目标',
			...
			boss_tenglong:'腾龙八齐',
			boss_tenglong_info:'你没有器具区；出牌阶段限一次，你可以弃置一张器具牌对一名距离你2以内的其他角色造成3点火焰伤害',
			boss_wushang:'神天并地',
